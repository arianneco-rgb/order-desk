...
//
// Rewritten 2026-07-29 after verifying against 7 real BPI notification
// emails (see BPI_MATCHING_HANDOFF.md) — the previous approach searched
// Gmail THREADS instead of individual messages. BPI sends every
// notification with the same subject and no threading headers, so Gmail
// collapsed the entire payment history into one giant thread and the old
// "only look at recent messages" filter did nothing. This version uses
// the Gmail advanced service (message-level, not thread-level) instead.
//
// SETUP:
//   1. script.google.com → New project (under the account that receives
//      BPI transfer emails).
//   2. Paste this whole file in.
//   3. Editor → Services (+ next to "Services") → Gmail API → Add.
//      Without this, every Gmail.* call below throws ReferenceError.
//   4. Fill in BPI_TRANSACTIONS_SHEET_ID below (ask whoever runs Order
//      Desk for it — it's a small shared spreadsheet, not a mailbox).
//   5. Run `installLogTrigger` once from the editor (▶ button, pick it
//      from the function dropdown). Accept the Gmail + Sheets permission
//      prompt — that's you granting the script access to your own inbox
//      and to that one shared sheet.
//   6. Done. It re-checks every 10 minutes on its own from then on — nothing
//      else to deploy or maintain.
//
// To verify it's working, run `_verify` from the editor and check the log
// (View → Logs, or Cmd/Ctrl+Enter).
